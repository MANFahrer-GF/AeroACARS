...
pub struct LandingScoringInput {
    ...
    // mit `score_algorithm_version = Some(4)` markiert (v0.16.21;
    // v0.12.0 war Float-Toleranz-Refinement mit `Some(3)`; vor
    // v0.12.0 war es `Some(2)`). Wenn eines fehlt → Skip mit
    // konkretem Reason (KEIN Fallback auf v1).
    //
...
pub fn compute_sub_scores(input: &LandingScoringInput) -> Vec<SubScoreEntry> {
    ...
    // Records dann mit `score_algorithm_version = Some(4)` (v0.16.21;
    // v0.12.0 war Float-Toleranz-Refinement mit `Some(3)`; vor
    // v0.12.0 war es `Some(2)`).
    //
