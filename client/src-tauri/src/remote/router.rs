...
//!
//! The MSFS in-sim panel does NOT use this server — see `crate::panel_server`
//! for its own always-on, fixed-port, loopback-only server. (v0.1.1 briefly
//! put panel routes here instead; moved out in v0.2.0/round 2b once reusing
//! this opt-in, configurable-port server turned out to cause exactly the
//! friction dropping the panel's PIN was meant to remove — see
//! `panel_server`'s module doc for the full story.)
